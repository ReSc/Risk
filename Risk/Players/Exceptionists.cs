...
namespace Risk.Players
{
    public class Exceptionists : IPlayer
    {
        ...
        public void Deploy(int numberOfTroops)
        {
            ...
            var countriesToAimAt = gameInformation.GetAllCountriesOwnedByPlayer(this).Where(x => gameInformation.GetAdjacentCountriesWithSameOwner(x).Any()).ToArray();
            ...
            var countriesWithEnemyNeighbours = countriesToAimAt.Where(x => gameInformation.GetAdjacentCountriesWithDifferentOwner(x).Count > 0).ToArray();
            var defensiveRates = GetDefensiveRates(countriesWithEnemyNeighbours).OrderBy(x => x.Value);

            while (defensiveRates.First().Value < DEFENSIVE_THRESHOLD_FOR_DEPLOYMENT && numberOfTroops > 0)
            {
                new TurnManager(this).DeployTroops(defensiveRates.First().Key, 1);
            ...
            KeyValuePair<EContinent, double> biggestContinent = GetBiggestOwnedContinentWithRemainingEnemyCountries();

            var allOwnCountriesInBiggestContinent = countriesToAimAt.Where(x => x.Continent == biggestContinent.Key).ToArray();
            var countriesWithRates = GetDefensiveRates(allOwnCountriesInBiggestContinent).Where(x => x.Key.NumberOfTroops < MAX_TROOPS_FOR_DEPLOYMENT).OrderByDescending(x => x.Value);

            while (countriesWithRates.Any() && numberOfTroops > 0)
            {
                new TurnManager(this).DeployTroops(countriesWithRates.First().Key, 1);
                countriesWithRates = GetDefensiveRates(allOwnCountriesInBiggestContinent).Where(x => x.Key.NumberOfTroops < MAX_TROOPS_FOR_DEPLOYMENT).OrderByDescending(x => x.Value);
                numberOfTroops--;
            }
        ...
        public void Attack()
        {
            ...
            {
                new TurnManager(this).Attack(countryToAttackWith.Item1, countryToAttackWith.Item2, countryToAttackWith.Item1.NumberOfTroops - 1);
        ...
        private Tuple<Country, Country, double> GetCountryToAttackWith()
        {
            ...
            var countriesWithHighEnoughDefenseRate = defensiveRates.Where(x => x.Value > DEFENSIVE_THRESHOLD_FOR_ATTACK).ToArray();
            ...
            foreach (var country in countriesWithHighEnoughDefenseRate)
            {
                foreach (var enemyCountry in gameInformation.GetAdjacentCountriesWithDifferentOwner(country.Key))
                {
                    ...
                    {
                        countriesToAttackWith.Add(new Tuple<Country, Country, double>(country.Key, enemyCountry, offenceRate));
                    }
                }
            }
        ...
        public void Move()
        {
            ...
            if (countriesToAimAt.Any())
            {
                ...
                while (defensiveRates.Any(x => x.Value < DEFENSIVE_THRESHOLD_FOR_DEPLOYMENT) && moveCounter++ < 250)
                {
                    ...
                    var adjacentCountriesWithSameOwner = gameInformation.GetAdjacentCountriesWithSameOwner(first.Key).Where(x => defensiveRates.Single(y => y.Key == x).Value > DEFENSIVE_THRESHOLD_FOR_DEPLOYMENT).ToArray();
                    if (adjacentCountriesWithSameOwner.Any())
                    {
                        new TurnManager(this).MoveTroops(first.Key, adjacentCountriesWithSameOwner.First(), adjacentCountriesWithSameOwner.First().NumberOfTroops - 1);
                    }
        ...
        private KeyValuePair<EContinent, double> GetBiggestOwnedContinentWithRemainingEnemyCountries()
        {
            ...
            {
                numberOfCountriesPerContinent.Add(continent, gameInformation.GetAllCountries().Count(x => x.Continent == continent));
            }

            var continentsWithOwnPresence = gameInformation.GetAllCountriesOwnedByPlayer(this).Select(x => x.Continent).Distinct();
            var numberOfOwnCountriesPerContinent = new Dictionary<EContinent, int>();
            foreach (var continent in continentsWithOwnPresence)
            {
                var currentNumber = gameInformation.GetAllCountriesOwnedByPlayer(this).Count(x => x.Continent == continent);
            ...
            foreach (var continent in continentsWithOwnPresence)
            {
                percentageOfOwnCountriesInContinent.Add(continent, (double)numberOfOwnCountriesPerContinent.Single(x => x.Key == continent).Value / (double)numberOfCountriesPerContinent.Single(x => x.Key == continent).Value);
            }
        ...
        private Dictionary<Country, double> GetDefensiveRates(IEnumerable<Country> countriesToUse)
        {
            ...
            {
                countriesWithOwnTroupEnemyTroupsRate.Add(country, GetDefensiveRateForCountry(country, GetNumberOfTotalEnemyTroupsForCountry(country)));
            }
        ...
        {
            var numberOfEnemies = gameInformation.GetAdjacentCountriesWithDifferentOwner(country).Sum(x => x.NumberOfTroops);
