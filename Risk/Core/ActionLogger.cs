...
namespace Risk.Core
{
    public class ActionLogger
    {
        ...
        {
            var existingDeployActions = gameManager.Actions.Where(x => x is DeployAction).Cast<DeployAction>();
            var existingActionThisTurn = existingDeployActions.FirstOrDefault(x => x.CountryToDeploy == country && x.ShowOnMap);
        ...
        public void AddException(Exception e, IPlayer player)
        {
            ...
            {
                Player = player,
                Message = string.Format("@@@ {0} caused the following exception: {1}", gameManager.CurrentPlayer.Name, e.Message)
            });
        }
